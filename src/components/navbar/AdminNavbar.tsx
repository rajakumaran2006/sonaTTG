...
const AdminNavbar = () => {
  ...
  const SidebarContent = () => (
    <div className={`flex h-full flex-col border-r shadow-sm transition-colors duration-300 ${isDark ? 'bg-gray-950 border-gray-800' : 'bg-white border-slate-100'}`}>
      ...
      {/* Department Switcher — shown when admin has at least one department */}
      {allocatedDepts.length > 0 && (
        <div className="px-4 pb-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild disabled={allocatedDepts.length <= 1}>
              <button className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-xl border text-left transition-all duration-200 text-sm font-medium ${
                allocatedDepts.length <= 1 ? 'cursor-default' : 'cursor-pointer hover:bg-gray-800'
              } ${isDark ? 'bg-gray-900 border-gray-800 text-gray-200' : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100'}`}>
                ...
                {allocatedDepts.length > 1 && <ChevronDown className="h-3.5 w-3.5 shrink-0 opacity-50" />}
              </button>
            </DropdownMenuTrigger>
