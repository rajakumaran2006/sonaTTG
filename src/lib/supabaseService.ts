...
// Check if a subject-section combination is already occupied by another faculty
export async function checkSubjectSectionConflict(
  departmentId: string,
  subjectId: string,
  year: string,
  section: string,
  excludeFacultyId?: string
): Promise<{ isOccupied: boolean; facultyName?: string }> {
  try {
    let query = (supabase as any)
      .from('faculty_subject_class')
      .select('faculty_id, faculty_members!inner(name)')
      .eq('department_id', departmentId)
    ...
    if (excludeFacultyId) {
      query = query.neq('faculty_id', excludeFacultyId);
    }
    ...
    if (data && data.length > 0) {
      return {
        isOccupied: true,
        facultyName: data[0].faculty_members?.name
      };
    }
    
    return { isOccupied: false };
  } catch (error) {
    console.warn('Failed to check subject-section conflict:', error);
    return { isOccupied: false };
  }
}

// Assign faculty to specific subject-section with conflict checking
export async function assignFacultyToSubjectSection(
  facultyId: string,
  departmentId: string,
  subjectId: string,
  year: string,
  ...
  try {
    ...
    const conflict = await checkSubjectSectionConflict(departmentId, subjectId, year, section, facultyId);
    
    if (conflict.isOccupied) {
      return {
        success: false,
        error: `This subject-section is already occupied by ${conflict.facultyName}`
      };
    }
    ...
    console.error('Failed to assign faculty to subject-section:', error);
    return {
      success: false,
