...
create policy if not exists "Faculty are publicly readable"
  on public.faculty_members for select using (true);
create policy if not exists "Faculty are publicly insertable"
  on public.faculty_members for insert with check (true);
create policy if not exists "Faculty are publicly updatable"
  on public.faculty_members for update using (true);
create policy if not exists "Faculty are publicly deletable"
...
-- Maintain updated_at
create trigger if not exists update_faculty_members_updated_at
